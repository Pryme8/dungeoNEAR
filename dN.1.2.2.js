...
dN.Mix = function (a, b, p){
	if(typeof a === 'object'){
	return {r:a.r*(1-p)+b.r*p,g:a.g*(1-p)+b.g*p,b:a.b*(1-p)+b.b*p, a:a.a*(1-p)+b.a*p};
	}else{
	return a*(1-p)+b*p;
	}
};
dN.Multiply = function (a, b){
	if(typeof a === 'object'){
	return {r:(a.r*b.r)*0.5, g:(a.g*b.g)*0.5, b:(a.b*b.b)*0.5, a:(a.a*b.a)*0.5};
	}else{
	return (a*b)/2;
	}
...
dN.filter = {
	...
	smooth : function(amount, value){
		if(typeof value === 'object'){
			return {r:value.r*amount, g:value.g*amount, b:value.b*amount, a:value.a*amount};
		}else{
			return value*amount;
		}
		
	},
	terrace: function(input, zones, power){
		for(var i = 0; i<zones.length; i++){
			var min = zones[i][0], max = zones[i][1];
